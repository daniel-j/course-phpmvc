<?php
/**
 * A model for managing Cloudchaser modules.
 * 
...
class CMModules extends CObject {
   ...
   * Properties
   */
  private $cloudchaserCoreModules = array('CCloudchaser', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $cloudchaserCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
  ...
  public function ReadAndAnalyse() {
    $src = CLOUDCHASER_INSTALL_PATH.'/src';
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isCloudchaserCore']   = in_array($rc->name, $this->cloudchaserCoreModules);
      $details['isCloudchaserCMF']    = in_array($rc->name, $this->cloudchaserCMFModules);
