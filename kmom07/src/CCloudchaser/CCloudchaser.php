...
class CCloudchaser implements ISingleton {
  ...
  public function ThemeEngineRender() {
    ...
    $themePath  = CLOUDCHASER_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = CLOUDCHASER_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl  = $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $cc->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $cc
    $this->themeUrl = $themeUrl;
    $this->themeParentUrl = $parentUrl;
    
    // Include the global functions.php and the functions.php that are part of the theme
    $cc = &$this;
    // First the default Cloudchaser themes/functions.php
    ...
    // Extract $this->data to own variables and handover to the template file
  ...
  public function DrawMenu($menu) {
    ...
    if(isset($this->config['menus'][$menu])) {
      foreach($this->config['menus'][$menu] as $item) {
        if (isset($item['controller'])) {
          $html .= "<li";
          if ($this->request->controller === $item['controller']) {
            $html .= " class='current'";
          }
          $html .= "><a href=\"".$this->request->CreateUrl($item['controller'])."\">{$item['text']}</a></li>\n";  
          ...
          foreach ($item['items'] as $item) {
            if (isset($item['controller'])) {
              $html .= "<li";
              $html .= "><a href=\"".$this->request->CreateUrl($item['controller'], isset($item['method'])?$item['method']:null)."\">{$item['text']}</a></li>\n";  
            } else if (isset($item['url'])) {
              $html .= "<li>"."<a href=\"".$item['url']."\">".$item['text']."</a>"."</li>\n";
            } else {
              $html .= "<li>???</li>";
      ...
      throw new Exception('No such menu '.$menu.'.');
    }     
    return "<nav id='{$menu}' class='menu'><ul>\n{$html}</ul></nav>\n";
  }
